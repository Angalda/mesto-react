      ...
      <>
        <div className={`pop-up pop-up_type_${props.name} ${props.isOpen ? 'pop-up_opened' : ''}`}>
          <div className={`pop-up__container pop-up__container-${props.name}`}>
            <button className="pop-up__closed" type="button" onClick={props.closePopup}></button>
            <form action="URL" className={`pop-up__form pop-up__form_${props.name}`} method="get" name={props.name} noValidate>
